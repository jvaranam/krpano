...
function loadJsonDatas(_param) {
    ...
    if (secInfoArr[1] == "119") {
        ...
        promiseArr.push(new LoadJson(resaleJsonPath + "/" + _param + ".json"));
    }
...
var ticketScaleVal = 0.07

function resaleHotspotsMng(_flag, _resaleData) {
    var krPanoObj = getPanoObj();
    if (_flag) {
        for (var i = 0; i < _resaleData.length; i++) {
            var scaleVal = _resaleData[i].d * ticketScaleVal;
            addHotspot(krPanoObj, _resaleData[i], 'HSP_R', scaleVal, 1, true, false);
        }
    } else {
        for (var i = 0; i < _resaleData.length; i++) {
            krPanoObj.call("removehotspot(" + _resaleData[i].n + ");");
        }
    }
}

function seatHotspotsMng(_flag, _seatData) {
    var krPanoObj = getPanoObj();
    if (_flag) {
        for (var i = 0; i < _seatData.length; i++) {
            var scaleVal = _seatData[i].d * ticketScaleVal;
            addHotspot(krPanoObj, _seatData[i], 'HSP', scaleVal, 1, true, false);
        }

        for (var i = 0; i < _seatData.length; i++) {
            var scaleVal = _seatData[i].d * ticketScaleVal;
            addHotspot(krPanoObj, _seatData[i], 'HSP_C', scaleVal, 0, false, false, 'selected');
        }
        ...
        for (var i = 0; i < _seatData.length; i++) {
            krPanoObj.call("removehotspot(" + _seatData[i].n + ");");
            krPanoObj.call("removehotspot(selected_" + _seatData[i].n + ");");
        }
    }
}
...
$('#switchOnOff').on('change.bootstrapSwitch', function(e, state) {
    ...
    if (e.target.checked) {
        seatHotspotsMng(false, seatData);
        resaleHotspotsMng(true, resaleData);
    } else {
        seatHotspotsMng(true, seatData);
        resaleHotspotsMng(false, resaleData);
    }
});
